# 🚜 디지털농업기계개론 실습 | Introduction to Digital Agricultural Machinery

> **이중 언어 저장소 (Bilingual Repository)**  
> 본 저장소는 한국어와 영어 두 가지 언어로 제공됩니다.  
> This repository is available in both Korean and English.

---

## 🇰🇷 한국어 (Korean)

**[📖 한국어 버전으로 이동 →](ko/README.md)**

본 저장소는 **디지털농업기계개론** 교과의 주차별 프로그래밍 실습 코드를 통합 관리하기 위한 공식 포트폴리오입니다.

| 주차 | 주제 | 폴더 |
|------|------|------|
| Week 04 | GNSS 정밀 측위 & A-B Line 경로 계획 | [`ko/week4/`](ko/week4/) |
| Week 05 | 광학 센서 & NDVI 맵핑 | [`ko/week5/`](ko/week5/) |
| Week 06 | 3D 공간 인식 — LiDAR 스캐닝 & 표현형 분석 | [`ko/week6/`](ko/week6/) |
| Week 07 | 스마트팜 환경 센싱 및 PID 제어 | [`ko/week7/`](ko/week7/) |

> 📝 **[주차별 심화 토론 & 퀴즈 모음](ko/QUIZ_BANK.md)**

---

## 🇺🇸 English

**[📖 Go to English Version →](en/README.md)**

This repository is the official portfolio for the **Introduction to Digital Agricultural Machinery** lab course, consolidating weekly programming assignments and simulation code.

| Week | Topic | Folder |
|------|-------|--------|
| Week 04 | GNSS Precision Positioning & A-B Line Path Planning | [`en/week4/`](en/week4/) |
| Week 05 | Optical Sensors & NDVI Mapping | [`en/week5/`](en/week5/) |
| Week 06 | 3D Spatial Perception — LiDAR Scanning & Phenotyping | [`en/week6/`](en/week6/) |
| Week 07 | Smart Farm Sensing & PID Control | [`en/week7/`](en/week7/) |

> 📝 **[Discussion Topics & Quiz Bank](en/QUIZ_BANK.md)**

---

## 📁 Repository Structure

```text
DigitalAgrMachinery/
├── README.md          ← Language Selection (this file)
├── ko/                ← 한국어 실습 콘텐츠 (Korean Content)
│   ├── README.md      ← 한국어 메인 포트폴리오
│   ├── QUIZ_BANK.md   ← 심화 토론 & 퀴즈
│   ├── week4/         ← [4주차] GNSS 정밀 측위 & A-B Line 경로 계획
│   ├── week5/         ← [5주차] 광학 센서 & NDVI
│   ├── week6/         ← [6주차] LiDAR 3D 스캐닝 & 표현형 분석
│   └── week7/         ← [7주차] 스마트팜 PID 제어
├── en/                ← English Content
│   ├── README.md      ← English Main Portfolio
│   ├── QUIZ_BANK.md   ← Discussion & Quiz Bank
│   ├── week4/         ← [Week 4] GNSS Positioning & A-B Line Path Planning
│   ├── week5/         ← [Week 5] Optical Sensors & NDVI
│   ├── week6/         ← [Week 6] LiDAR 3D Scanning & Phenotyping
│   └── week7/         ← [Week 7] Smart Farm PID Control
└── .gitignore
```

---

*© 2026 Introduction to Digital Agricultural Machinery Lab Course — Jeonbuk National University*
