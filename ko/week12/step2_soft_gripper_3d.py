"""
step2_soft_gripper_3d.py — 3D 소프트 그리퍼 인터랙티브 시뮬레이션

3개의 소프트 핑거가 120도 간격으로 배치되어 3차원 공간에서
과일(구체)을 향해 뻗어나가며, 압력에 따라 안쪽으로 굽어
과일을 감싸는 모습을 슬라이더로 조절하여 실시간 관찰

핑거 동작 원리:
  - 핑거는 그리퍼 베이스에서 X+ 방향(전방)으로 뻗음
  - 각 핑거는 YZ 평면에서 120도 간격으로 배치 (gap 반지름)
  - 압력 증가 → 핑거가 중심축(과일 방향)으로 안쪽 굽힘

조작 슬라이더:
  - Pressure: 공압 (굽힘 정도)
  - Fruit X: 과일 전후 위치
  - Gripper X: 그리퍼 전후 위치
  - Finger Gap: 핑거-중심축 간 거리

의존성: numpy, matplotlib (추가 패키지 불필요)
"""
...
def generate_finger_3d(pressure, gripper_x, gap, theta):
    ...
    핑거는 X 방향(전방)으로 뻗으며, 압력에 따라
    중심축(X축) 방향으로 안쪽 굽힘

    :param pressure:  입력 공압 (0.0 ~ 1.0)
    :param gripper_x: 그리퍼 베이스 X 좌표
    :param gap:       핑거 시작점의 중심축 거리 (반지름)
    :param theta:     YZ 평면에서의 배치 각도 (라디안)
    :return: (x, y, z) 좌표 배열
    """
    length = 0.7
    n_seg = 30
    ...
    # 최대 곡률 — 압력 1.0일 때 약 150도 굽힘 (안쪽으로)
    max_k = np.pi / length * 0.85
    k = pressure * max_k

    # 2D 곡선: 전방(forward) vs 반지름(radial) 평면에서 생성
    # forward = X축 방향, radial = 중심축으로부터의 거리
    fwd = [0.0]     # X축 방향 누적
    rad = [gap]     # 반지름 방향 (gap에서 시작, 안쪽으로 줄어듦)
    angle = 0.0
    ...
    # 3D 변환: forward → X축, radial → YZ 평면 (theta 방향)
    x = gripper_x + fwd
    y = rad * np.cos(theta)
    z = rad * np.sin(theta)
